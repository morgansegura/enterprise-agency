...
function resolveConfig<T>(
  ...
  if (typeof value === "string") {
    ...
    if (!config) {
      console.warn(`${type} reference "${value}" not found in site config`);
      return null;
    }
    return config;
  }
  ...
  if (!defaultConfig) {
    console.warn(`Default ${type} "${defaultKey}" not found in site config`);
    return null;
  }
...
export function resolveMenu(
  ...
  if (!menu) {
    console.warn(`Menu reference "${menuRef}" not found in site config`);
    return null;
  }
