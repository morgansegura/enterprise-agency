...
['admin', 'users', 'search', query]         // Search results
['admin', 'users', userId]                  // Single user

// Features
['admin', 'features', 'available']          // Available features
['admin', 'features', 'tenant', tenantId]   // Tenant features

// Projects
['admin', 'projects', 'assignments']                    // All assignments
['admin', 'projects', 'assignments', { tenantId }]      // Filtered by tenant
['admin', 'projects', 'assignments', { userId }]        // Filtered by user
['admin', 'projects', 'assignments', assignmentId]      // Single assignment

// Tenants
['admin', 'tenants']                        // All tenants
['admin', 'tenants', tenantId]              // Single tenant
['admin', 'tenants', tenantId, 'stats']     // Tenant stats
['admin', 'tenants', tenantId, 'activity']  // Tenant activity
```
...
export function useToggleFeature() {
  ...
  return useMutation({
    mutationFn: ({ tenantId, featureKey, enabled }: {
      tenantId: string
      featureKey: string
      enabled: boolean
    }) =>
...
export function useProjectAssignments(filters?: {
  ...
  return useQuery({
    ...
    queryFn: () =>
      apiClient.get('/admin/projects/assignments', filters),
  })
}
...
export function useProjectAssignment(assignmentId: string) {
  return useQuery({
    ...
    queryFn: () =>
      apiClient.get(`/admin/projects/assignments/${assignmentId}`),
  })
}
