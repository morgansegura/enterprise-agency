...
export function useProjectAssignments(filters?: ProjectAssignmentFilters) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      logger.log('Fetched project assignments', { count: data.length, filters })
      return data
    },
  })
}
...
export function useCreateAssignment() {
  ...
  return useMutation({
    mutationFn: async (data: CreateProjectAssignmentDto) => {
      ...
      return apiClient.post<ProjectAssignment>('/admin/projects/assignments', data)
    },

    onSuccess: (newAssignment) => {
      logger.log('Assignment created successfully', { assignmentId: newAssignment.id })
