...
export function useAvailableFeatures() {
  return useQuery({
    ...
    queryFn: async () => {
      const data = await apiClient.get<FeatureDefinition[]>('/admin/features/available')
...
export function useUpdateFeatures() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      logger.log('Updating tenant features', { tenantId, featureCount: Object.keys(features).length })
...
export function useToggleFeature() {
  ...
  return useMutation({
    ...
    onSuccess: (data, { tenantId, featureKey, enabled }) => {
      logger.log('Feature toggled successfully', { tenantId, featureKey, enabled })
