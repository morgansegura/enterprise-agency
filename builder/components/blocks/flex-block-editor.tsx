...
interface FlexBlockData {
  ...
  data: {
    ...
    blocks: Block[];
    _responsive?: {
      tablet?: Partial<Omit<FlexBlockData["data"], "blocks">>;
      mobile?: Partial<Omit<FlexBlockData["data"], "blocks">>;
    };
  };
}
...
export function FlexBlockEditor({
  ...
  const handleDataChange = useResponsiveChange(block.data, (newData) =>
    onChange({ ...block, data: newData as FlexBlockData["data"] }),
  );
  ...
  const direction =
    getResponsiveValue<string>(block.data, "direction", breakpoint) || "row";
