...
export function GlobalSettingsDrawer({
  ...
  const [originalTokens, setOriginalTokens] = React.useState<typeof localTokens>({
    colors: defaultColors,
    typography: {
      fontFamily: { base: "Open_Sans", heading: "Roboto_Slab" },
      borderRadius: "0.5rem",
    ...
    if (tokens) {
      ...
      const initial = {
        colors: {
          ...
          primaryForeground: semanticTokens.semantic?.primaryForeground || defaultColors.primaryForeground,
          secondary: semanticTokens.semantic?.secondary || defaultColors.secondary,
          secondaryForeground: semanticTokens.semantic?.secondaryForeground || defaultColors.secondaryForeground,
          accent: semanticTokens.semantic?.accent || defaultColors.accent,
          accentForeground: semanticTokens.semantic?.accentForeground || defaultColors.accentForeground,
          background: semanticTokens.semantic?.background || defaultColors.background,
          foreground: semanticTokens.semantic?.foreground || defaultColors.foreground,
          card: semanticTokens.semantic?.card || defaultColors.card,
          cardForeground: semanticTokens.semantic?.cardForeground || defaultColors.cardForeground,
          popover: semanticTokens.semantic?.popover || defaultColors.popover,
          popoverForeground: semanticTokens.semantic?.popoverForeground || defaultColors.popoverForeground,
          muted: semanticTokens.semantic?.muted || defaultColors.muted,
          mutedForeground: semanticTokens.semantic?.mutedForeground || defaultColors.mutedForeground,
          ...
          destructive: semanticTokens.semantic?.destructive || defaultColors.destructive,
        },
        typography: {
          fontFamily: {
          ...
          borderRadius: (tokens.borderRadius as { base?: string })?.base || "0.5rem",
        },
      };
  ...
  return (
    <SettingsDrawer open={open} onOpenChange={onOpenChange} title="Global Settings">
      <SettingsDrawerSidebar
      ...
      <SettingsDrawerContent isLoading={isLoading}>
        ...
        {activeTab === "colors" && (
          <ColorSettings colors={localTokens.colors} onChange={handleColorChange} />
        )}
        {activeTab === "typography" && (
          <TypographySettings
            fontFamily={localTokens.typography?.fontFamily || {}}
            borderRadius={localTokens.typography?.borderRadius || "0.5rem"}
            ...
            {(Object.keys(tailwindColors) as ColorName[]).map((colorName) => (
              <div key={colorName} className="color-row">
                ...
                <div className="color-row-shades">
                  {(Object.keys(tailwindColors[colorName]) as unknown as ShadeKey[]).map((shade) => {
                    const hex = tailwindColors[colorName][shade];
                    return (
                      <button
                        key={shade}
                        className={cn("color-swatch", value === hex && "is-selected")}
...
type SemanticColorKey =
  ...
  | "muted" | "mutedForeground" | "border" | "input" | "ring" | "destructive";
...
function ColorSettings({ colors, onChange }: ColorSettingsProps) {
  return (
    <SettingsSection
      title="Brand Colors"
      description="Configure all semantic color tokens for this website. These will be saved and applied globally to the site."
    >
      <SettingsGridBlock title="Brand">
        <ColorPicker label="Primary" value={colors.primary} onChange={(v) => onChange("primary", v)} />
        <ColorPicker label="Primary Foreground" value={colors.primaryForeground} onChange={(v) => onChange("primaryForeground", v)} />
        <ColorPicker label="Secondary" value={colors.secondary} onChange={(v) => onChange("secondary", v)} />
        <ColorPicker label="Secondary Foreground" value={colors.secondaryForeground} onChange={(v) => onChange("secondaryForeground", v)} />
        <ColorPicker label="Accent" value={colors.accent} onChange={(v) => onChange("accent", v)} />
        <ColorPicker label="Accent Foreground" value={colors.accentForeground} onChange={(v) => onChange("accentForeground", v)} />
      </SettingsGridBlock>

      <SettingsGridBlock title="Backgrounds">
        <ColorPicker label="Background" value={colors.background} onChange={(v) => onChange("background", v)} />
        <ColorPicker label="Foreground" value={colors.foreground} onChange={(v) => onChange("foreground", v)} />
        <ColorPicker label="Card" value={colors.card} onChange={(v) => onChange("card", v)} />
        <ColorPicker label="Card Foreground" value={colors.cardForeground} onChange={(v) => onChange("cardForeground", v)} />
        <ColorPicker label="Popover" value={colors.popover} onChange={(v) => onChange("popover", v)} />
        <ColorPicker label="Popover Foreground" value={colors.popoverForeground} onChange={(v) => onChange("popoverForeground", v)} />
      </SettingsGridBlock>

      <SettingsGridBlock title="UI Elements">
        <ColorPicker label="Muted" value={colors.muted} onChange={(v) => onChange("muted", v)} />
        <ColorPicker label="Muted Foreground" value={colors.mutedForeground} onChange={(v) => onChange("mutedForeground", v)} />
        <ColorPicker label="Border" value={colors.border} onChange={(v) => onChange("border", v)} />
        <ColorPicker label="Input" value={colors.input} onChange={(v) => onChange("input", v)} />
        <ColorPicker label="Ring (Focus)" value={colors.ring} onChange={(v) => onChange("ring", v)} />
      </SettingsGridBlock>

      <SettingsGridBlock title="Status">
        <ColorPicker label="Destructive" value={colors.destructive} onChange={(v) => onChange("destructive", v)} />
      </SettingsGridBlock>
    </SettingsSection>
  );
}
...
function TypographySettings({ fontFamily, borderRadius, onFontChange, onRadiusChange }: TypographySettingsProps) {
  return (
    <SettingsSection
      title="Typography & Style"
      description="Configure fonts and border radius for this website. Fonts are loaded via Next.js Google Fonts for optimal performance."
    >
      <SettingsForm>
        <SettingsField>
          <Label htmlFor="font-base">Base Font (Body Text)</Label>
          <Select value={fontFamily.base || "Open_Sans"} onValueChange={(v) => onFontChange("base", v)}>
            <SelectTrigger id="font-base">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <div className="px-2 py-1.5 text-xs font-semibold text-(--muted-foreground)">Sans-serif</div>
              {googleFonts.filter((f) => f.category === "sans").map((font) => (
                <SelectItem key={font.value} value={font.value}>{font.label}</SelectItem>
              ))}
              <div className="px-2 py-1.5 text-xs font-semibold text-(--muted-foreground) border-t mt-1 pt-2">Serif</div>
        ...
        <SettingsField>
          ...
          <Select value={fontFamily.heading || "Roboto_Slab"} onValueChange={(v) => onFontChange("heading", v)}>
